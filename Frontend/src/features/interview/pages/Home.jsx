    ...
    if (loading) {
        return (
            <main className='min-h-screen bg-[#02000d] flex flex-col items-center justify-center p-6 text-center'>
                ...
                <h1 className='text-3xl font-black text-white tracking-tighter mb-2'>Architecting Strategy...</h1>
                <p className='text-gray-500 font-medium animate-pulse'>Neural networks are analyzing your profile against job requirements.</p>
            </main>
        )
    }
    ...
    return (
        <div className='min-h-screen bg-[#02000d] text-slate-200 font-sans selection:bg-pink-500/30 pb-20'>
            ...
            <header className='relative z-10 max-w-6xl mx-auto pt-16 pb-12 px-6 text-center md:text-left'>
                ...
                >
                    Forge Your <br />
                    <span className='text-transparent bg-clip-text bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-500'>Interview Roadmap</span>
                </motion.h1>
                <motion.p 
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                ...
                >
                    Supply the parameters below. Our AI will dissect the requirements and architect a high-performance execution plan.
                </motion.p>
            </header>
                        ...
                        <div className="grid md:grid-cols-2 gap-10">
                            ...
                            <div className='space-y-6'>
                                <div className='flex items-center gap-3'>
                                    ...
                                    <div>
                                        <h2 className='text-lg font-bold text-white'>Job Intelligence</h2>
                                        <p className='text-[10px] text-gray-500 font-black uppercase tracking-widest'>Target Requirements</p>
                                    </div>
                                </div>
                                <textarea
                            ...
                            <div className='space-y-6'>
                                <div className='flex items-center gap-3'>
                                    ...
                                    <div>
                                        ...
                                        <p className='text-[10px] text-gray-500 font-black uppercase tracking-widest'>Candidate Data</p>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                    <label className='block group cursor-pointer'>
                                        <div className='relative border-2 border-dashed border-white/10 rounded-2xl p-8 text-center hover:border-indigo-500/50 hover:bg-indigo-500/[0.02] transition-all'>
                                            <input ref={resumeInputRef} hidden type='file' accept='.pdf,.docx' onChange={handleFileChange} />
                                            <Upload className='mx-auto mb-3 text-gray-500 group-hover:text-indigo-400 group-hover:-translate-y-1 transition-all' size={32} />
                                            <p className='text-sm font-bold text-white mb-1'>{fileName || "Upload Resume"}</p>
                                            <p className='text-[10px] text-gray-500 font-bold uppercase tracking-tighter'>PDF or DOCX (Max 5MB)</p>
                                        </div>
                                    </label>
                        ...
                        <div className="mt-12 pt-8 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-6">
                            <div className="flex items-start gap-3 max-w-sm">
                                ...
                                <p className="text-[11px] text-gray-500 leading-relaxed">
                                    Our engine requires either a <span className="text-white font-bold">Resume</span> or <span className="text-white font-bold">Self Description</span> paired with the Job Intel to proceed.
                                </p>
                            </div>
                            <button
                                onClick={handleGenerateReport}
                                className='w-full md:w-auto px-10 py-4 bg-white text-black font-black rounded-2xl hover:bg-pink-600 hover:text-white transition-all flex items-center justify-center gap-3 group shadow-[0_0_30px_rgba(255,255,255,0.1)] hover:shadow-pink-500/40'
                            >
                                GENERATE STRATEGY
                ...
                <aside className='lg:col-span-4 space-y-8'>
                                ...
                                {reports.slice(0, 4).map(report => (
                                    ...
                                    >
                                        <div className='flex justify-between items-start mb-2'>
                                            <h3 className='font-bold text-white text-sm group-hover:text-pink-400 transition-colors truncate pr-4'>{report.title || 'Untitled Position'}</h3>
                                            <ChevronRight className="text-gray-700 group-hover:text-white transition-colors" size={16} />
                                        </div>
                                        <div className='flex justify-between items-center'>
                                            <span className='text-[10px] font-black text-gray-600 uppercase tracking-widest'>{new Date(report.createdAt).toLocaleDateString()}</span>
                                            <span className='text-[11px] font-black text-pink-500'>{report.matchScore}%</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
                    ...
                    <div className="p-8 bg-gradient-to-br from-indigo-600/20 to-purple-600/20 border border-indigo-500/20 rounded-[2.5rem] relative overflow-hidden">
                        ...
                        <p className="text-sm text-indigo-200/70 leading-relaxed relative z-10">
                            The more specific the Job Intelligence you provide, the more surgical our AI can be with your roadmap. Paste the full description!
                        </p>
                    </div>
                </aside>
            </main>
            ...
            <footer className='max-w-6xl mx-auto px-6 mt-20 pt-10 border-t border-white/5 flex flex-wrap justify-center gap-8 text-[10px] font-black text-gray-700 uppercase tracking-[0.3em]'>
                ...
                <a href='#' className="hover:text-white transition-colors">Systems Help</a>
            </footer>
        </div>
    )
}
