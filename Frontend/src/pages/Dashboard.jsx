...
export default function Dashboard() {
  ...
  const avgScore = reports.length === 0 
    ? 0 
    : Math.round(reports.reduce((sum, r) => sum + r.matchScore, 0) / reports.length);
    ...
    <div className="flex min-h-screen bg-[#02000d] text-slate-200 selection:bg-pink-500/30 font-sans">
      ...
      <aside className="fixed left-0 top-0 h-screen w-20 md:w-64 border-r border-white/5 bg-[#030014]/50 backdrop-blur-2xl z-50 flex flex-col items-center md:items-stretch py-8 px-4">
        <div className="flex items-center gap-3 px-2 mb-12">
          ...
          <span className="hidden md:block text-xl font-black tracking-tighter text-white uppercase">Nexus<span className="text-pink-500">.</span></span>
        </div>

        <nav className="flex-1 space-y-2 w-full">
          <SidebarItem icon={<LayoutDashboard size={20}/>} label="Overview" active />
          <SidebarItem icon={<FileText size={20}/>} label="My Plans" />
          <SidebarItem icon={<Plus size={20}/>} label="New Strategy" onClick={() => navigate("/app")} highlight />
        </nav>
        ...
        <div className="relative z-10 max-w-6xl mx-auto">
          ...
          <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16">
            <div>
              <h1 className="text-4xl md:text-6xl font-black text-white tracking-tight mb-4">
                Systems <span className="text-transparent bg-clip-text bg-gradient-to-r from-gray-100 to-gray-500 italic font-light text-5xl">Active</span>
              </h1>
              <div className="flex items-center gap-4 text-gray-500 text-sm font-medium">
                <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"/> Engine: V3-Neural</span>
            ...
            <div className="flex bg-white/5 border border-white/10 rounded-2xl p-1.5 backdrop-blur-md">
               <button onClick={() => navigate("/app")} className="px-6 py-3 bg-white text-black font-bold rounded-xl hover:bg-pink-500 hover:text-white transition-all flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            ...
            <div className="md:col-span-1 bg-gradient-to-br from-indigo-600 to-violet-700 rounded-[2rem] p-8 relative overflow-hidden group shadow-lg shadow-indigo-500/20">
              <div className="relative z-10">
                <p className="text-white/70 text-[10px] font-black uppercase tracking-widest mb-2">Pro Feature</p>
                <h3 className="text-2xl font-bold text-white mb-4 leading-tight tracking-tight">AI Mock Interviews</h3>
              ...
              <Zap size={140} className="absolute right-[-20px] bottom-[-20px] text-white/10 -rotate-12 group-hover:scale-110 transition-transform duration-700" />
            </div>
          </div>
          ...
          <section>
            <div className="flex items-center justify-between mb-8">
               <h2 className="text-2xl font-bold text-white tracking-tight">Intelligence Logs</h2>
               <div className="relative hidden md:block">
                 <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500" size={14} />
                 <input className="bg-white/5 border border-white/10 rounded-xl py-2 pl-10 pr-4 text-xs focus:outline-none focus:border-pink-500/50 w-64 transition-all" placeholder="Search logs..." />
               </div>
            </div>
            ...
            <div className="space-y-3">
              <AnimatePresence>
                {reports.length > 0 ? reports.map((report, i) => (
                  ...
                  <motion.div initial={{opacity: 0}} animate={{opacity: 1}} className="py-20 border border-dashed border-white/10 rounded-[2rem] flex flex-col items-center justify-center text-gray-500">
                    <FileText size={32} className="mb-4 opacity-20" />
                    <p className="font-bold tracking-tight text-sm uppercase">No intelligence logs found.</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
...
function SidebarItem({ icon, label, active = false, highlight = false, onClick }) {
  return (
    <button 
      onClick={onClick}
  ...
  return (
    <div className="group bg-white/[0.03] border border-white/5 rounded-[2rem] p-8 hover:bg-white/[0.06] hover:border-white/10 transition-all duration-500 shadow-xl">
      <div className="flex justify-between items-start mb-6">
        <div className={`p-3 rounded-2xl bg-white/5 text-${color}-400 group-hover:scale-110 transition-transform duration-500`}>
          {icon}
        </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-5 w-full md:w-auto">
        ...
        <div>
          <h4 className="text-lg font-bold text-white group-hover:text-pink-400 transition-colors tracking-tight">{report.title || "Standard Analysis"}</h4>
          <p className="text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em]">
            Log: {report._id.slice(-6)} • {new Date(report.createdAt).toLocaleDateString()}
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-8 mt-4 md:mt-0 w-full md:w-auto justify-between md:justify-end">
        <div className="text-right">
          <div className="flex items-center gap-3">
             ...
             <span className="text-2xl font-black text-white">{report.matchScore}%</span>
          </div>
          <p className="text-[10px] text-gray-500 uppercase font-black tracking-tighter">Compatibility</p>
        </div>
